...
cat ${output_path}_knowpse.txt ${output_path}_notpse.txt > ${output_path}_svm_plot.txt

nohup Rscript $(dirname "$0")/svm_build_totalRNA.r -f ${output_path}_svm_plot.txt -k ${output_path}_svm_filt_totalRNA.bed -r $(dirname "$0")/hg38_human_chr21_rRNA_known_pseudoU_SingleSites.bed -s $(dirname "$0")/hg38.psiU.SingleSites.bed  -o ${output_path} > ${output_path}_svm_evaluation_totalRNA.log 2>&1 &
wait

cat ${output_path}_svm_evaluation_totalRNA.log
# mupdf-x11 ${output_path}_six_variables_rRNA_violinplot.pdf &> /dev/null
mupdf-x11 ${output_path}_svm_roc_test_data_plot.pdf &> /dev/null
mupdf-x11 ${output_path}_svm_evaluation.pdf &> /dev/null

echo "total RNA: svm build and predict programs are done!"
echo -e "total RNA: svm prediction result in $(dirname ${output_path}_svm_psi_prediction.bed)"
