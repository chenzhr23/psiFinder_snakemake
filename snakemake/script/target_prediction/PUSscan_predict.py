...
to_predict = to_predict.replace(".bed",".txt")
to_pred = pd.read_csv(to_predict,sep="\t")
# Select the column 
sequences = to_pred['extendSeq_20nt']
# Loop through sequences and replace 21st char
for i in range(len(sequences)):
    if len(sequences[i]) >= 21:
        sequences[i] = sequences[i][:21] + "Y" + sequences[i][22:]
# Update column in dataframe        
to_pred['Y_extendSeq_20nt'] = sequences
